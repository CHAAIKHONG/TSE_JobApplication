...
// Protected pages
$protectedPages = ['profile.php', 'applications.php', 'settings.php', 'dashboard.php', 'notification.php', 'education.php'];
...
// Fetch user from DB only if not already set by host page
...
// Ensure initials & notif_count exist
...
if (!empty($_SESSION['user_id'])) {
    ...
    $nRes = $conn->query("SELECT COUNT(*) AS cnt FROM notifications WHERE user_id = $uid AND is_read = 0");
    if ($nRes) $unreadCount = (int)$nRes->fetch_assoc()['cnt'];
}

if (!empty($currentUser)) {
    $currentUser['notif_count'] = $unreadCount;
}
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ap-nav__logo-mark::after {
    ...
    width: 14px; height: 14px; background: var(--accent); border-radius: 6px 0 0 0;
  }
  ...
  .ap-nav__links { display: flex; align-items: center; gap: 4px; list-style: none; }

  .ap-nav__links a {
    text-decoration: none; color: var(--mid); font-size: 14px; font-weight: 500;
  ...
  /* Clickable (logged in) */
  .ap-nav__notif--active { cursor: pointer; }
  .ap-nav__notif--active:hover { border-color: var(--ink); background: var(--ink); }
  .ap-nav__notif--active:hover svg { stroke: #fff; }

  /* Disabled (not logged in) */
  .ap-nav__notif--disabled {
    cursor: not-allowed;
    opacity: 0.4;
    pointer-events: none;
  }
  ...
  .ap-nav__notif svg {
    width: 16px; height: 16px; stroke: var(--mid); fill: none;
    stroke-width: 1.8; transition: stroke 0.2s;
  }
  ...
  .ap-nav__notif-badge {
    ...
    border-radius: 8px; border: 1.5px solid var(--paper);
    display: flex; align-items: center; justify-content: center;
  ...
  .ap-nav__chevron {
    width: 14px; height: 14px; stroke: var(--mid); fill: none;
  ...
  .ap-nav__dropdown {
    ...
    box-shadow: 0 8px 24px rgba(0,0,0,0.08); display: none; z-index: 200;
  }
  ...
  .ap-nav__dropdown a {
    ...
    font-size: 13px; color: var(--mid); text-decoration: none; border-radius: 6px;
  ...
  .ap-nav__dropdown a.disabled { opacity: 0.4; pointer-events: none; cursor: default; }
...
<nav class="ap-nav" role="navigation" aria-label="Main navigation">
  <div class="ap-nav__inner">
    ...
    <a href="dashboard.php" class="ap-nav__logo" aria-label="ApplyGo">
    ...
    <div class="ap-nav__actions">
      ...
      <?php if ($loggedIn): ?>
        <a href="notification.php"
           class="ap-nav__notif ap-nav__notif--active"
          ...
          <?php if ($unreadCount > 0): ?>
            <span class="ap-nav__notif-badge">
              <?= $unreadCount > 9 ? '9+' : $unreadCount ?>
            </span>
          <?php endif; ?>
        </a>
      <?php else: ?>
        <!-- Not logged in: disabled bell, no href -->
        <span class="ap-nav__notif ap-nav__notif--disabled"
      ...
      <div class="ap-nav__profile" id="profileMenu">
        <button class="ap-nav__profile-btn" onclick="toggleProfile()"
        ...
        <div class="ap-nav__dropdown" role="menu">
          ...
          <a href="<?= $loggedIn ? 'profile.php' : 'login.php' ?>"
             <?= !$loggedIn ? 'class="disabled"' : '' ?> role="menuitem">
          ...
          <a href="<?= $loggedIn ? 'education.php' : 'login.php' ?>"
             <?= !$loggedIn ? 'class="disabled"' : '' ?> role="menuitem">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <a href="<?= $loggedIn ? 'notification.php' : '#' ?>"
             <?= !$loggedIn ? 'class="disabled"' : '' ?> role="menuitem">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/>
              <path d="M13.73 21a2 2 0 0 1-3.46 0"/>
            ...
            <?php if ($loggedIn && $unreadCount > 0): ?>
              <span style="margin-left:auto;background:var(--accent);color:#fff;font-size:10px;font-weight:700;padding:1px 6px;border-radius:10px;">
                <?= $unreadCount ?>
              </span>
            <?php endif; ?>
          </a>
